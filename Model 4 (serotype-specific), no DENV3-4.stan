...
data {
  ...
  array[nT, nA, 2] int cases; // reported case data
...
transformed data {
  ...
  array[2, nT] int sum_cases; // observed total cases per year
...
parameters {
  ...
  array[5] simplex[2] lam_H; // historic average serotype FOI proportions
  array[nT] simplex[2] lam_t; // time varying yeraly serotype FOI proportions 
...
transformed parameters {
  ...
  array[2] row_vector<lower=0, upper=1>[max_age] mono0; // proportion monotypic at time 0
  ...
  array[2] matrix<lower=0, upper=1>[nT, max_age] mono; // proportion monotypic
  matrix<lower=0, upper=1>[nT, max_age] multi; // proportion multitypic
  array[2] matrix<lower=0, upper=1>[nT, max_age] inc1; // incidence of primary infections
  array[2] matrix<lower=0, upper=1>[nT, max_age] inc2; // incidence of secondary infections
  array[nT, 2] vector<lower=0>[nA] Ecases; // expected reported cases per year and age group
  array[2] vector<lower=0>[nT] Ecases_sum; // expected reported cases per year
  array[nT, 2] vector<lower=0>[nA] prob_Ecases; // expected probability of reported cases per year and age group
...
model {
  ...
  //--- priors
  row_vector[2] alpha = [1, 1]; //so dirichlet prior symmetric
  for (t in 1:nT) {
  ...
  for(s in 1 : 2) for (t in 1 : nT) target += poisson_lpmf(sum_cases[s, t] | Ecases_sum[s, t]);

  // multinomial likelihood for age and year

  for(s in 1 : 2) for (t in 1 : nT) target += multinomial_lpmf(cases[t,  : , s] | prob_Ecases[t, s, : ]);

}

generated quantities {
 
  array[2] vector[nT] log_lik;
  array[2] vector[nT] log_lik1;
  array[2] vector[nT] log_lik2;
 
  for(s in 1 : 2) for (t in 1 : nT)     log_lik1[s, t] = poisson_lpmf(sum_cases[s, t] | Ecases_sum[s, t]);
 
  for(s in 1 : 2) for (t in 1 : nT)     log_lik2[s, t] = multinomial_lpmf(cases[t,  : , s] | prob_Ecases[t, s, : ]);
 
  for(s in 1 : 2)  for (t in 1 : nT)     log_lik[s, t] = log_lik1[s, t] + log_lik2[s, t];
